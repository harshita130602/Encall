...
const Options = (props) => {
  ...
  // click option handling
  const handleClick = (event) => {
    setAnchorEl(event.currentTarget);
  };

  // Handling close option
  const handleClose = () => {
    setAnchorEl(null);
  };
  // If call is received and not yet picked up then call else we have set it to false
  useEffect(() => {
    ...
    <>
      <div className={showEditor ? 'options' : 'options w100'}>
        <button className='tooltip' onClick={handleClick}>
          <MoreVertIcon />
          <span className='tooltiptext'>Options</span>              {/*Options */}
        </button>
        ...
        >
          {myVideoStatus ? <VideocamIcon /> : <VideocamOffIcon />}    {/* Video on/Off */}
        ...
        >
          <CallEndIcon />
          <span className='tooltiptext'>End call</span>                 {/*End call */}
        </button>
            ...
            {!mobileView && (
              ...
              >
                ...
                <AntSwitch
                  ...
                  onChange={() => {
                    message.info(
                      `Switched to ${showEditor ? 'normal' : 'interview'} mode`                 // switch to interview mode
                    );
              ...
              <div className='call-div'>
                ...
                <div className='flex'>
                  <Button
                    type='primary'
                    onClick={() => {
                      answerCall();                                           // answercall function called
                      setOpen(false);
                    }}
                  >
                    Accept
